...
import org.jbehavesupport.engine.story.NestedGivenStories
...
class NestedGivenStoryTest extends Specification {
    ...
    def "Test correct notifications"() {
        ...
        executionResults.allEvents()
            .assertEventsMatchExactly(
                ...
                event(container(NestedGivenStories), started()),
                event(test("BeforeStories"), started()),
                event(test("BeforeStories"), finishedSuccessfully()),
                event(container("Story: NestedGivenStory"), started()),
                ...
                event(test("GivenStoryWithNestedGivenStory"), started()),
                event(test("GivenStoryWithNestedGivenStory"), finishedSuccessfully()),
                ...
                event(container("Story: NestedGivenStory"), finishedSuccessfully()),
                event(test("AfterStories"), started()),
                event(test("AfterStories"), finishedSuccessfully()),
                event(container(NestedGivenStories), finishedSuccessfully()),
    ...
    def "Test descriptions"() {
        ...
        then:
        desc.displayName == "NestedGivenStories"
        children.size() == 3
        children[0].displayName =~ /BeforeStories.*/
        children[1].displayName == "Story: NestedGivenStory"
        children[1].children[0].displayName == "Scenario: Given story test"
        children[1].children[0].children[0].displayName =~ /.*GivenStory1/
        children[1].children[0].children[1].displayName =~ /.*GivenStoryWithNestedGivenStory/
    ...
    def "Test correct notifications for story level reporter"() {
        ...
        executionResults.allEvents()
            .assertEventsMatchExactly(
                event(engine(), started()),
                event(container(NestedGivenStories), started()),
                event(test("NestedGivenStory"), started()),
                event(test("NestedGivenStory"), finishedSuccessfully()),
                event(container(NestedGivenStories), finishedSuccessfully()),
                event(engine(), finishedSuccessfully())
            )
    }
    ...
    def "Test descriptions for story level reporter"() {
        ...
        desc.displayName == "NestedGivenStories"
        children.size() == 1
        children[0].displayName =~ "Story: NestedGivenStory"
